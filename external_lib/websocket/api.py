# api.py
# call:  uvicorn external_lib.websocket.api:fastapi_app --host 0.0.0.0 --port 8888
import sys
import time
import logging
import base64
...
def register_routes(fastapi_app:FastAPI):
    ...
    def video_feed(source_name: str):
        publish_frame(source_name, None)  # initialize thread-safe, returns whether source_name is initialized
        try:
...
def run_api(logger, host="0.0.0.0", port=8888):
    web = VideoWebApp(host=host, port=port)
    fastapi_app = init_fastapi_app(logger=logger)
    if fastapi_app is not None:
        register_routes(fastapi_app=fastapi_app)
    return web, fastapi_app
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(f"Failed to start uvicorn process (Error: {error})")
