...
def run_video_stream(status, io_buff_in, cmd_words, trace):
    ...
    keywords = {
        ...
        "grpc_name": ("str", False, False, True)     # Specify ip:port of inference endpoint
    }
    ...
    if not ret_val:
        ...
        if connect_name not in connected_:
            ...
            grpc_name = interpreter.get_one_value(conditions, "grpc_name")  # The name of the connection
            ret_val = connected_[connect_name].start_stream_video(status=status, external_func=enabled_libs, grpc_name=grpc_name)
...
def exit_video(status, io_buff_in, cmd_words, trace):
    ...
    if not ret_val:
        ...
        if not connect_name in connected_:
            ...
            ret_val = connected_[connect_name].graceful_exit(status=status)        # cal graceful exit
