...
class SSEConnection:
    ...
    def send_event(self, event_type: str, data: Dict[str, Any], event_id: Optional[int] = None) -> bool:
        ...
        try:
            ...
            # Expected when client disconnects - log at debug level
            logger.debug(f"Client disconnected during event send to {self.session_id}: {e}")
            self.connected = False
            return False
        except Exception as e:
            # Unexpected error - log at warning level
            logger.warning(f"Unexpected error sending SSE event to {self.session_id}: {e}")
            self.connected = False
            return False
    ...
    def send_keepalive(self) -> bool:
        ...
        try:
            ...
            if hasattr(e, 'errno') and e.errno == 32:  # Broken pipe
                logger.debug(f"Client disconnected during keepalive: {self.session_id}")
            else:
                logger.debug(f"Connection lost during keepalive to {self.session_id}: {e}")
            self.connected = False
            return False
        except Exception as e:
            # Unexpected error - log at warning level
            logger.warning(f"Unexpected error sending keepalive to {self.session_id}: {e}")
            self.connected = False
            return False
