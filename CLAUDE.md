...
<rule id="3" use following format for error logging:                      
     err_msg = "{}"
     status.add_error(err_msg)
<rule id="4">Use Mel all after code change when you want to build and test. Mel all runs build, deploy, start-mcp-server and test-sse-tools</rule>
<rule id="5">CRITICAL: Never load entire large files (>500 lines) into context without user approval. For large files like member_cmd.py (17,000+ lines), params.py (65,000+ lines), or http_server.py (3,000+ lines):
